...
def get_test_response():
    ...
    session_attributes = {}
    card_title = "Test"
    speech_output = "Esta es una prueba"
    reprompt_text = "You never responded to the first test message. Sending another one."
    should_end_session = False
    return build_response(session_attributes, build_speechlet_response(
        card_title, speech_output, reprompt_text, should_end_session))

def us_get_welcome_response():
    """ If we wanted to initialize the session to have some attributes we could
    add those here
    """
    session_attributes = {}
    card_title = "Welcome"
    speech_output = "Greetings from Bronx fool"
...
def mx_get_welcome_response():
    """ If we wanted to initialize the session to have some attributes we could
    add those here
    """
    session_attributes = {}
    card_title = "Bienvenido"
    speech_output = "Saludos de Monterrey!"
    # If the user either does not reply to the welcome message or says something
    # that is not understood, they will be prompted again with this text.
    reprompt_text = "I don't know if you heard me, welcome to your custom alexa application!"
    should_end_session = False
...
def get_insult_response(intent, session):
    ...
    if name == "my_enemy":
        speech_output = 'Te voy a decir una cosa <p><amazon:effect name="whispered">{name} es un maldito clasista.</amazon:effect>.</p>'.format(name=name)
    else:
        speech_output = 'La mamá de {name} es una llama'.format(name=name)
...
def on_launch(launch_request, session):
    ...
    if launch_request["locale"] == "en-US":
        return us_get_welcome_response()
    elif launch_request["locale"]== "es-MX":
        return mx_get_welcome_response()
